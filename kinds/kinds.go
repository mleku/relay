...
//
// Note that the request must use the typed kind.T or convert the number thus. Even if a custom
// number is found, this codebase does not have the logic to deal with the kind so such a search
// is pointless and for which reason static typing always wins. No mistakes possible with known
// quantities.
