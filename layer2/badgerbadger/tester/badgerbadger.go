...
func main() {
	...
	os.RemoveAll(path)
	b1 := ratel.GetBackend(c, &wg, true, true, units.Gb, lol.Error, 4*units.Mb,
		"none")
	// create L2 with no cache management
	b2 := ratel.GetBackend(c, &wg, false, true, units.Gb, lol.Trace,
		4*units.Mb, "none")
	...
	for {
		...
		go func() {
			...
			for {
				select {
				case <-newEvent:
					// make new request, not necessarily from existing... bias rng factor by
					// request count
					mx.Lock()
					var sum int
					for i := range counter {
						...
						// multiply this number by the number of accesses the event has and
						// request every event that gets over 50% so that we create a bias
						// towards already requested.
