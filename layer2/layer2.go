// Package layer2 is a library for building nostr event stores with two separate data storage
// systems, primarily for creating size limited caches with larger stores backing them, to
// enable scaling providing access to an event store to more users more quickly via a caching
// strategy.
package layer2
...
type Backend struct {
	...
	L2 store.I
	// PollFrequency is how often the L2 is queried for recent events. This is only relevant for
	// shared layer2 stores, and will not apply for layer2 implementations that are just two
	// separate data store systems on the same server.
	PollFrequency time.Duration
	// PollOverlap is the multiple of the PollFrequency within which polling the L2 is done to
	// ensure any slow synchrony on the L2 is covered (2-4 usually).
	PollOverlap int
	// EventSignal triggers when the L1 saves a new event from the L2
	//
	// caller is responsible for populating this so that a signal can pass to all peers sharing
	// the same L2 and enable cross-cluster subscription delivery.
	EventSignal event.C
}
...
func (b *Backend) Init(path string) (err error) {
	b.path = path
	// each backend will have configuration files living in a subfolder of the same root,
	// path/layer1 and path/layer2 - this may only be state/configuration, or it can be the site
