// Package eventid is a codec for managing nostr event Ids (hash of the canonical form of a
// nostr event).
package eventid
...
// T is the SHA256 hash in hexadecimal of the canonical form of an event as produced by the
// output of T.ToCanonical().Bytes().
...
// NewWith creates an eventid.T out of bytes or string but assumes it is binary and that it is
// the right length. The result is either truncated or padded automatically by the use of the
// "copy" operation.
...
// NewFromString inspects a string and ensures it is a valid, 64 character long hexadecimal
// string, returns the string coerced to the type.
