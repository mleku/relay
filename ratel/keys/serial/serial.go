// Package serial implements a keys.Element for encoding a serial (monotonic 64 bit counter) for
// stored events, used to link an index to the main data table.
package serial
...
// New returns a new serial record key.Element - if nil or short slice is given, initialize a
// fresh one with Len (for reading), otherwise if equal or longer, trim if long and store into
// struct (for writing).
...
// Match compares a key bytes to a serial, all indexes have the serial at the end indicating the
// event record they refer to, and if they match returns true.
