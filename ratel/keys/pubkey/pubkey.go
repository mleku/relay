...
// New creates a new pubkey prefix, if parameter is omitted, new one is allocated (for read) if
// more than one is given, only the first is used, and if the first one is not the correct
// hexadecimal length of 64, return error.
