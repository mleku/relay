...
func assertConversationKeyGeneration(t *testing.T, priv, pub, conversationKey string) bool {
	...
	if ok = assert.Equalf(t, expectedConversationKey, actualConversationKey,
		"wrong conversation key"); !ok {
		return false
	}
	return true
}
...
func assertMessageKeyGeneration(t *testing.T,
	...
	if ok = assert.Equalf(t, expectedChaChaNonce, actualChaChaNonce,
		"wrong encrypt nonce"); !ok {
		return false
	}
