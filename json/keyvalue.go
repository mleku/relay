...
func (k *KeyValue) Unmarshal(dst []byte) (rem []byte, err error) {
	...
	k.Key = s.V
	// note that we aren't checking there isn't gobbledygook between the sentinels... this code
	// would allow:
	//
	//   `"key"abcdefghij-literally anything:\nliterally\t \f anythingelse \r"string"`
	//
	// but once the data is in what does it matter, if it's valid. and such garbage won't be
	// part of a canonical form to generate a hash so that still works.
