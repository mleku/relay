// Package filter is a codec for nostr filters (queries) and includes tools for matching them to
// events, a canonical format scheme to enable compactly identifying subscription filters, and a
// simplified filter that leavse out the IDs and Search fields for use in the HTTP API.
package filter
...
// canonicalized so that a hash can identify the same filter. This does not tolerate whitespace.
// Fall back to encoding/json.Unmarshal for this case.
...
// Unmarshal a filter from raw (minified) JSON bytes into the runtime format. This does not
// tolerate whitespace. Fall back to encoding/json.Unmarshal for this case.
