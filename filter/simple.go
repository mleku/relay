...
// S is a simplified filter that only covers the nip-01 REQ filter minus the separate and
// superseding Id list. The search field is from a different NIP, but it is a separate API for
// which reason it is also not here.
type S struct {
...
// created by Marshal.
//
// The purpose of this fingerprint is enabling the creation of a map of filters that can be
// searched by making a canonical form of a provided input filter that will match the same
// filter. It achieves this by making all fields sorted in lexicographical order and from this a
// single 8 byte truncated hash can be used to identify if a received filter is the same filter.
