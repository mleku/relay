// Package event provides a codec for nostr events, for the wire format (with Id and signature),
// for the canonical form, that is hashed to generate the Id, and a custom format called
// "wirecompact" which wraps a canonical form with an array and encodes the signature with
// base64 for a more compact size.
package event
...
import (
	...
	"relay.mleku.dev/unix"

	"relay.mleku.dev/chk"
	"relay.mleku.dev/errorf"
)

// T is the primary datatype of nostr. This is the form of the structure that defines its JSON
// string based format.
type T struct {
	...
	Pubkey []byte
	// CreatedAt is the UNIX timestamp of the event according to the event creator (never trust
	// a timestamp!)
	CreatedAt *timestamp.T
	// Kind is the nostr protocol code for the type of event. See kind.T
	Kind *kind.T
	// Tags are a list of tags, which are a list of strings usually structured as a 3 layer
	// scheme indicating specific features of an event.
	Tags *tags.T
	// Content is an arbitrary string that can contain anything, but usually conforming to a
	// specification relating to the Kind and the Tags.
	Content []byte
	// Sig is the signature on the Id hash that validates as coming from the Pubkey in binary
	// format.
	Sig []byte
}
...
// stringy/number functions for retarded other libraries and http input with whitespace.
...
// GenerateRandomTextNoteEvent creates a generic event.T with random text content.
