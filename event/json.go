...
// marshalWithWhitespace adds tabs and newlines to make the JSON more readable for humans, if
// the on flag is set to true.
...
// Marshal is a normal function that is the same as event.T Marshal method except you explicitly
...
func (ev *T) Unmarshal(b []byte) (r []byte, err error) {
	// this parser does not cope with whitespaces in valid places in json, so we scan first for
	// linebreaks, as these indicate that it is probably not going to work and fall back to
	// json.Unmarshal
	for _, v := range b {
		if v == '\n' {
...
// Unmarshal is the same as the event.T Unmarshal method except you give it the event to marshal
// into instead of call it as a method of the type.
