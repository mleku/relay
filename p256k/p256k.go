...
//
// Either the Sec or Pub must be populated, the former is for generating signatures, the latter
// is for verifying them.
//
// When using this library only for verification, a constructor that converts from bytes to
// PubKey is needed prior to calling Verify.
type Signer struct {
