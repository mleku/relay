...
// FromSecretBytes parses and processes what should be a secret key. If it is a correct key
// within the curve order, but with a public key having an odd Y coordinate, it returns an error
// with the fixed key.
...
// Generate gathers entropy to generate a full set of bytes and CGO values of it and derived
// from it to perform signature and ECDH operations.
func Generate() (
	skb, pkb []byte,
	sec *Sec,
	pub *XPublicKey,
...
// Keygen is an implementation of a key miner designed to be used for vanity key generation with
// X-only BIP-340 keys.
type Keygen struct {
...
// NewKeygen allocates the required buffers for deriving a key. This should only be done once to
// avoid garbage and make the key mining as fast as possible.
//
// This allocates everything and creates proper CGO variables needed for the generate function
// so they only need to be allocated once per thread.
...
// Generate takes a pair of buffers for the secret and ec pubkey bytes and gathers new entropy
// and returns a valid secret key and the compressed pubkey bytes for the partial collision
// search.
