...
func (p *Page) Layout(gtx C, th *material.Theme) D {
	...
	return material.List(th, &p.List).Layout(gtx, 1, func(gtx C, _ int) D {
		return layout.Flex{
			...
			layout.Rigid(func(gtx C) D {
				return alo.DefaultInset.Layout(gtx,
					material.Body2(th, "Responds to hover events.").Layout)
			}),
			layout.Rigid(func(gtx C) D {
			...
			layout.Rigid(func(gtx C) D {
				return alo.DefaultInset.Layout(gtx, material.Body2(th,
					"Label animates properly when you click to select the text field.").Layout)
			}),
			layout.Rigid(func(gtx C) D {
			...
			layout.Rigid(func(gtx C) D {
				return alo.DefaultInset.Layout(gtx,
					material.Body2(th, "Can have prefix and suffix elements.").Layout)
			}),
			layout.Rigid(func(gtx C) D {
				if err := func() string {
			...
			layout.Rigid(func(gtx C) D {
				return alo.DefaultInset.Layout(gtx,
					material.Body2(th, "Can have a character counter and help text.").Layout)
			}),
			layout.Rigid(func(gtx C) D {
			...
			layout.Rigid(func(gtx C) D {
				return alo.DefaultInset.Layout(gtx, material.Body2(th,
					"This text field implementation was contributed by Jack Mordaunt. Thanks Jack!").Layout)
			}),
		)
	})
}
