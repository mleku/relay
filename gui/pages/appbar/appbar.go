		...
		return layout.Flex{
			...
			layout.Rigid(func(gtx C) D {
				return alo.DefaultInset.Layout(gtx, material.Body1(th,
					`The app bar widget provides a consistent interface element for triggering navigation and page-specific actions.
...
The controls below allow you to see the various features available in our App Bar implementation.`).Layout)
			...
			layout.Rigid(func(gtx C) D {
				return alo.DetailRow{}.Layout(gtx,
					material.Body1(th, "Contextual App Bar").Layout, func(gtx C) D {
			...
			layout.Rigid(func(gtx C) D {
				return alo.DetailRow{}.Layout(gtx,
					...
					func(gtx C) D {
						...
						return material.Switch(th, &p.bottomBar,
							"Use Bottom App Bar").Layout(gtx)
					})
			}),
			layout.Rigid(func(gtx C) D {
				return alo.DetailRow{}.Layout(gtx,
					...
					func(gtx C) D {
						...
						return material.Switch(th, &p.customNavIcon,
							"Use Custom Navigation Icon").Layout(gtx)
					})
			}),
			layout.Rigid(func(gtx C) D {
				return alo.DetailRow{}.Layout(gtx,
					...
					material.Body2(th,
						"Resize the width of your screen to see app bar actions collapse into or emerge from the overflow menu (as size permits).").Layout,
				)
			}),
			layout.Rigid(func(gtx C) D {
				return alo.DetailRow{}.Layout(gtx,
					...
					material.Body2(th,
						"Click the heart action to see custom button behavior.").Layout)
			}),
		)
	})
}
