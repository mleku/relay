...
import (
	...
	"gio.mleku.dev/gio/io/clipboard"
	"gio.mleku.dev/gio/layout"
	"gio.mleku.dev/gio/widget"
	"gio.mleku.dev/gio/widget/material"

	alo "relay.mleku.dev/gui/applayout"
	"relay.mleku.dev/gui/icon"
	page "relay.mleku.dev/gui/pages"
)
	...
	return material.List(th, &p.List).Layout(gtx, 1, func(gtx C, _ int) D {
		return layout.Flex{
			...
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return alo.DefaultInset.Layout(gtx, material.Body1(th,
					`This library implements material design components from https://material.io using https://gioui.org.
...
If you like this library and work like it, please consider sponsoring Elias and/or Chris!`).Layout)
			...
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return alo.DetailRow{}.Layout(gtx,
					material.Body1(th,
						"Elias Naur can be sponsored on GitHub at "+sponsorEliasURL).Layout,
					func(gtx C) D {
						...
						return material.Button(th, &p.eliasCopyButton,
							"Copy Sponsorship URL").Layout(gtx)
					})
			}),
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return alo.DetailRow{}.Layout(gtx,
					material.Body1(th,
						"Chris Waldon can be sponsored on GitHub at "+sponsorChrisURLGitHub+" and on Liberapay at "+sponsorChrisURLLiberapay).Layout,
					...
					func(gtx C) D {
						...
						return alo.DefaultInset.Layout(gtx, func(gtx C) D {
							return layout.Flex{}.Layout(gtx,
								layout.Flexed(.5, material.Button(th, &p.chrisCopyButtonGH,
									"Copy GitHub URL").Layout),
								layout.Flexed(.5, material.Button(th, &p.chrisCopyButtonLP,
									"Copy Liberapay URL").Layout),
							)
						})
					})
			}),
