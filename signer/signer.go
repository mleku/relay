// Package signer defines interfaces for management of signatures, used to abstract the
// signature algorithm from the usage.
package signer
...
type I interface {
	...
	// ECDH works).
	Generate() (err error)
	// InitSec initialises the secret (signing) key from the raw bytes, and also derives the
	// public key because it can.
	InitSec(sec []byte) (err error)
	// InitPub initializes the public (verification) key from raw bytes, this is expected to be
	...
	Zero()
	// ECDH returns a shared secret derived using Elliptic Curve Diffie-Hellman on the I secret
	// and provided pubkey.
