// Package tag provides an implementation of a nostr tag list, an array of strings with a
// usually single letter first "key" field, including methods to compare, marshal/unmarshal and
// access elements with their proper semantics.
package tag
...
//
// The last element is treated specially in that it is considered to match if the candidate has
// the same initial substring as its corresponding element.
