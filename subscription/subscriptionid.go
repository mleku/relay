// Package subscription is a set of helpers for managing nostr websocket subscription Ids, used
// with the REQ method to maintain an association between a REQ and resultant messages such as
// EVENT and CLOSED.
...
// IsValid returns true if the subscription id is between 1 and 64 characters. Invalid means too
// long or not present.
func (si *Id) IsValid() bool { return len(si.T) <= 64 && len(si.T) > 0 }

// NewId inspects a string and converts to Id if it is valid. Invalid means length == 0 or
// length > 64.
