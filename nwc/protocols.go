...
// Interfaces
//
// By using these interfaces and embedding the following implementations it becomes simple to
// type check the specific request, response or notification variable being used in a given
// place in the code, without using reflection.
//
// All request, responses and methods embed the implementations and their types then become
// easily checked.
...
// Implementations
//
// By embedding the following types into the message structs and writing a constructor that
// loads the type name, code can handle these without reflection, determine type via type
// assertion and introspect the message type via the interface accessor method.
...
// Msat is milli-sat, max possible value is 1000 x 21 x 100 000 000 (well, under 19 places of 64
// bits in base 10)
type Msat uint64
