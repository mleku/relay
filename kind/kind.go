// Package kind includes a type for convenient handling of event kinds, and a kind database with
// reverse lookup for human-readable information about event kinds.
package kind
...
// T - which will be externally referenced as kind.T is the event type in the nostr protocol,
// the use of the capital T signifying type, consistent with Go idiom, the Go standard library,
// and much, conformant, existing code.
type T struct {
	K uint16
}
...
// IsEphemeral returns true if the event kind is an ephemeral event. (not to be stored)
func (k *T) IsEphemeral() bool {
	return k.K >= EphemeralStart.K && k.K < EphemeralEnd.K
}

// IsReplaceable returns true if the event kind is a replaceable kind - that is, if the newest
// version is the one that is in force (eg follow lists, relay lists, etc.
func (k *T) IsReplaceable() bool {
	return k.K == ProfileMetadata.K || k.K == FollowList.K ||
		(k.K >= ReplaceableStart.K && k.K < ReplaceableEnd.K)
}

// IsParameterizedReplaceable is a kind of event that is one of a group of events that replaces
...
// Directory events are events that necessarily need to be readable by anyone in order to
// interact with users who have access to the relay, in order to facilitate other users to find
// and interact with users on an auth-required relay.
var Directory = []*T{
	ProfileMetadata,
	FollowList,
	EventDeletion,
...
// IsDirectoryEvent returns whether an event kind is a Directory event, which should grant
// permission to read such events without requiring authentication.
...
var (
	// ProfileMetadata is an event type that stores user profile data, pet names, bio, lightning
	// address, etc.
	ProfileMetadata = &T{0}
	...
	RecommendServer = &T{2}
	RecommendRelay  = &T{2}
	// FollowList an event containing a list of pubkeys of users that should be shown as follows
	// in a timeline.
	FollowList = &T{3}
	Follows    = &T{3}
	...
	BadgeAward = &T{8}
	// Seal is an event that wraps a PrivateDirectMessage and is placed inside a GiftWrap or
	// GiftWrapWithKind4
	Seal = &T{13}
	// PrivateDirectMessage is a nip-17 direct message with a different construction. It doesn't
	// actually appear as an event a relay might receive but only as the stringified content of
	// a GiftWrap or GiftWrapWithKind4 inside a
	PrivateDirectMessage = &T{14}
	// ReadReceipt is a type of event that marks a list of tagged events (e tags) as being seen
	// by the client, its distinctive feature is the "expiration" tag which indicates a time
	// after which the marking expires
	ReadReceipt = &T{15}
	...
	EmojiSets            = &T{30030}
	// ApplicationSpecificData is an event type stores data about application configuration,
	// this, like DMs and gift-wraps must be protected by user auth.
