// Package eventenvelope is a codec for the event Submission request EVENT envelope (client) and
// event Result (to a REQ) from a relay.
...
// Marshal an event Submission envelope in minified JSON, appending to a provided destination
// slice.
...
// Unmarshal an event eventenvelope.Submission from minified JSON, returning the remainder after
...
// ParseSubmission reads an event envelope Submission from minified JSON into a newly allocated
...
// NewResultWith creates a new eventenvelope.Result with a provided subscription.Id string and
// event.T.
...
// Marshal an eventenvelope.Result envelope in minified JSON, appending to a provided
// destination slice.
...
// Unmarshal an event Result envelope from minified JSON, returning the remainder after the end
// of the envelope.
...
// ParseResult allocates a new eventenvelope.Result and unmarshalls an EVENT envelope into it.
