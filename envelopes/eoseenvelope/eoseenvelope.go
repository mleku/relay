...
// T is an EOSE envelope (End of Stored Events), that signals the end of events that are stored
// and the beginning of a subscription. This is necessitated by the confusing multiplexing of
// websockets for multiple requests, and an ugly merging of two distinct API calls, filter and
// subscribe.
type T struct {
...
// Marshal a eoseenvelope.T envelope in minified JSON, appending to a provided destination
// slice.
...
// Unmarshal a eoseenvelope.T from minified JSON, returning the remainder after the end of the
// envelope.
