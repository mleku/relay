// Package closeenvelope provides the encoder for the client message CLOSE which is a request to
...
// T is a CLOSE envelope, which is a signal from client to relay to stop a specified
// subscription.
type T struct {
	ID *subscription.Id
}
...
// Marshal a closeenvelope.T envelope in minified JSON, appending to a provided destination
// slice.
...
// Unmarshal a closeenvelope.T from minified JSON, returning the remainder after the end of the
// envelope.
