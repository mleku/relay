// Package countenvelope is an encoder for the COUNT request (client) and response (relay)
// message types.
...
// Request is a COUNT envelope sent by a client to request a count of results. This is a stupid
// idea because it costs as much processing as fetching the events, but doesn't provide the
// means to actually get them (the HTTP API /filter does this by returning the actual event
// Ids).
type Request struct {
...
// NewRequest creates a new Request with a provided subscription.Id and filter.T.
...
// Marshal a Request appended to the provided destination slice as minified JSON.
...
// Unmarshal a Request from minified JSON, returning the remainder after the end of the
// envelope.
...
// Response is a COUNT Response returning a count and approximate flag associated with the REQ
// subscription.Id.
...
// subscription.Id, a count and optional variadic approximate flag, which is otherwise false and
// does not get rendered into the JSON.
...
// Marshal a countenvelope.Response envelope in minified JSON, appending to a provided
// destination slice.
...
// Unmarshal a COUNT Response from minified JSON, returning the remainder after the end of the
// envelope.
...
// Parse reads a Count Response in minified JSON into a newly allocated countenvelope.Response.
