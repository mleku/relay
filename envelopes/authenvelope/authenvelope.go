...
import (
	...
	"relay.mleku.dev/codec"
	envs "relay.mleku.dev/envelopes"
	"relay.mleku.dev/event"
	"relay.mleku.dev/text"
...
// Challenge is the relay-sent message containing a relay-chosen random string to prevent replay
// attacks on NIP-42 authentication.
...
// Marshal a authenvelope.Challenge to minified JSON, appending to a provided destination slice.
// Note that this ensures correct string escaping on the challenge field.
...
// Unmarshal a authenvelope.Challenge from minified JSON, returning the remainder after the end
// of the envelope. Note that this ensures the challenge string was correctly escaped by NIP-01
// escaping rules.
...
// ParseChallenge reads a authenvelope.Challenge encoded in minified JSON and unpacks it to the
// runtime format.
...
// Response is a client-side envelope containing the signed event bearing the relay's URL and
// Challenge string.
...
// Marshal a Response to minified JSON, appending to a provided destination slice. Note that
// this ensures correct string escaping on the challenge field.
...
// Unmarshal a Response from minified JSON, returning the remainder after the en of the
// envelope. Note that this ensures the challenge string was correctly escaped by NIP-01
// escaping rules.
...
// ParseResponse reads a Response encoded in minified JSON and unpacks it to the runtime format.
