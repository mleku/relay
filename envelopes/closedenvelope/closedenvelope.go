// Package closedenvelope defines the nostr message type CLOSED which is sent from a relay to
// indicate the relay-side termination of a subscription or the demand for authentication
...
// T is a CLOSED envelope, which is a signal that a subscription has been stopped on the relay
// side for some reason. Primarily this is for auth and can be for other things like rate
// limiting.
type T struct {
...
// Marshal a closedenvelope.T envelope in minified JSON, appending to a provided destination
// slice. Note that this ensures correct string escaping on the Reason field.
...
// Unmarshal a closedenvelope.T from minified JSON, returning the remainder after the end of the
