// Package okenvelope is a codec for the OK message, which is an acknowledgement for an EVENT
// eventenvelope.Submission, containing true/false and if false a message with a
// machine-readable error type as found in the messages package.
package okenvelope
...
// T is an OK envelope, used to signal acceptance or rejection, with a reason, to an
// eventenvelope.Submission.
type T struct {
	EventID *eventid.T
	OK      bool
	Reason  []byte
...
// NewFrom creates a new okenvelope.T with a string for the subscription.Id and the Reason.
...
// Marshal a okenvelope.T from minified JSON, appending to a provided destination slice. Note
// that this ensures correct string escaping on the subscription.Id and Reason fields.
...
// Unmarshal a okenvelope.T from minified JSON, returning the remainder after the end of the
// envelope. Note that this ensures the Reason and subscription.Id strings are correctly
...
// Parse reads a OK envelope in minified JSON into a newly allocated okenvelope.T.
